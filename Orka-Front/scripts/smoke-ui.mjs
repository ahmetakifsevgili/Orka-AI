...
addCheck("Landing keeps learner-first product depth", landing.includes("Ders") && landing.includes("Kaynak") && landing.includes("Mini kontrol") && landing.includes("Telafi") && landing.includes("Not"));
...
addCheck(
  "Vite production chunks are explicitly split",
  viteConfig.includes("chunkSizeWarningLimit: 2300") &&
    viteConfig.includes("manualChunks") &&
    viteConfig.includes("vendor-mermaid") &&
    viteConfig.includes("vendor-code-editor") &&
    viteConfig.includes("vendor-graph"),
);
...
addCheck("Notebook Studio source notebook API exposed", api.includes("getTopicNotebook") && api.includes("/sources/topic/") && api.includes("/notebook") && api.includes("buildSourcePack") && api.includes("/notebook-studio/sources/") && types.includes("SourceNotebookDto") && types.includes("sourceSurface"));
addCheck("Notebook Studio source-concept graph API exposed", api.includes("getSourceConceptLinks") && api.includes("syncSourceConceptLinks") && api.includes("/concept-links/sync") && api.includes("getTopicSourceConceptGraph") && api.includes("/concept-graph") && api.includes("getWikiPageSourceLinks") && types.includes("SourceConceptGraphDto") && types.includes("SourceConceptLinkSummaryDto"));
addCheck("Notebook Studio ask-source API exposed", api.includes("askTopicSources") && api.includes("askSources") && api.includes("/sources/ask") && api.includes("/ask") && types.includes("SourceQuestionRequestDto") && types.includes("SourceQuestionResponseDto") && types.includes("SourceQuestionCitationDto"));
addCheck("Notebook Studio multi-source compare API exposed", api.includes("compareTopicSources") && api.includes("/compare") && api.includes("getTopicCitationReview") && api.includes("/citation-review") && types.includes("MultiSourceCompareResultDto") && types.includes("CitationReviewItemDto"));
addCheck("Notebook Studio source Q&A thread API exposed", api.includes("listQuestionThreads") && api.includes("createQuestionThread") && api.includes("askQuestionThread") && api.includes("reviewQuestionThread") && api.includes("writeQuestionThreadWikiTrace") && api.includes("/sources/question-threads") && types.includes("SourceQuestionThreadDto") && types.includes("SourceQuestionTurnDto"));
addCheck("Notebook Studio source study summary API exposed", api.includes("getSourceStudySummary") && api.includes("/sources/study-summary") && types.includes("SourceStudySummaryDto") && types.includes("recommendedNextAction"));
...
addCheck("Agentic workspace shows safe student rail", runtimeWorkspace.includes("Bu turda Orka") && runtimeWorkspace.includes("Kısa çalışma özeti") && runtimeWorkspace.includes("Kaynaklı açıklama") && !runtimeWorkspace.includes("Runtime sagligi") && !runtimeWorkspace.includes("rawProviderPayload") && !runtimeWorkspace.includes("rawToolPayload"));
...
addCheck("Sources / Wiki remains active while Notebook Studio reuses source surface", wiki.includes('mode?: "wiki" | "orkalm"') && wiki.includes("Source notebook") && wiki.includes("Notebook sources"));
addCheck("Notebook Studio dedicated source notebook UI visible", wiki.includes("Source notebook") && wiki.includes("activeSourceNotebook") && wiki.includes("sourceNotebook") && notebookStudio.includes("Build source set") && notebookStudio.includes("source_notebook"));
addCheck("Notebook Studio source-to-concept graph UI visible", wiki.includes("Source-to-concept graph") && wiki.includes("sourceConceptLinks") && wiki.includes("sourceConceptGraph") && wiki.includes("handleSyncSourceConceptLinks") && wiki.includes("Oneri grafigi"));
...
addCheck("Notebook Studio ask-source UX visible", wiki.includes("handleAskSource") && wiki.includes("handleAskSourceCollection") && wiki.includes("Ask selected source") && wiki.includes("Ask source collection") && wiki.includes("Citation chips") && wiki.includes("Related concept pages") && wiki.includes("sourceQuestionResponse"));
addCheck("Notebook Studio multi-source compare UX visible", wiki.includes("handleCompareSources") && wiki.includes("Multi-source compare") && wiki.includes("Compare selected") && wiki.includes("Shared linked concepts") && wiki.includes("Citation review") && wiki.includes("semantic agreement iddiasi uretmez"));
addCheck("Notebook Studio source Q&A memory UX visible", wiki.includes("Source Q&A memory") && wiki.includes("Save source Q&A thread") && wiki.includes("Ask follow-up") && wiki.includes("Mark needs review") && wiki.includes("Write to Wiki") && wiki.includes("activeQuestionThread"));
addCheck("Notebook Studio source study summary UX visible", wiki.includes("Source study status") && wiki.includes("sourceStudySummary") && wiki.includes("recommendedNextAction") && wiki.includes("citationWarningCount") && wiki.includes("linked concepts"));
addCheck("Notebook Studio ask-source UI is payload-safe", !wiki.includes("rawProviderPayload") && !wiki.includes("rawSourceChunk") && !wiki.includes("hiddenPrompt") && !wiki.includes("correctAnswer") && !wiki.includes("chunk.text.slice") && !wiki.includes("{chunk.text}"));
addCheck("Notebook Studio compare UI avoids fake agreement claims", !wiki.includes("semantic agreement") || wiki.includes("semantic agreement iddiasi uretmez"));
...
addCheck("Wiki study entry is visible", wiki.includes("Study entry") && wiki.includes("Work this topic") && wiki.includes("Özeti oku") && wiki.includes("Kavramları gözden geçir") && wiki.includes("Kartlarla çalış"));
addCheck("Wiki repair queue context is visible", wiki.includes("This topic has a repair queue signal.") && wiki.includes("Review the weak concept in Wiki or open a short repair question in Tutor.") && wiki.includes("wiki-study-reinforcement"));
...
addCheck("Wiki Vault page-aware Notebook context is preserved", wiki.includes("NotebookStudioPanel") && wiki.includes("wikiContextPageId") && wiki.includes("wikiPageId={wikiContextPageId}") && wiki.includes("wikiPageTitle={wikiContextPageId ? activePage?.title : undefined}"));
...
addCheck("Notebook Studio artifact panel is wired into Wiki", wiki.includes("NotebookStudioPanel") && notebookStudio.includes("Notebook Studio") && notebookStudio.includes("mind_map") && notebookStudio.includes("uml_diagram") && notebookStudio.includes("review_quiz"));
addCheck("Notebook Studio artifact production states are visible", notebookStudio.includes("Kaynak durumu") && notebookStudio.includes("Artifact sets could not be loaded") && notebookStudio.includes("audio_script") && notebookStudio.includes("No artifact yet."));
addCheck("Notebook Studio phase 2-4 professional preview is visible", notebookStudio.includes("ProfessionalArtifactPreview") && notebookStudio.includes("PropertiesMatrix") && notebookStudio.includes("GraphScopePreview") && notebookStudio.includes("TemplateSetPreview") && notebookStudio.includes("SearchFilterPreview") && notebookStudio.includes("SlideDeckPreview") && notebookStudio.includes("PhaseSevenAudioNotice"));
addCheck("Notebook Studio advanced non-audio/export actions are visible", notebookStudio.includes("slide_export_manifest") && notebookStudio.includes("uml_diagram") && notebookStudio.includes("graph_view") && notebookStudio.includes("search_filter_index") && notebookStudio.includes("Export readiness"));
addCheck("Notebook Studio audio actions are active with user-safe copy", notebookStudio.includes('  "audio_overview",') && notebookStudio.includes('  "audio_script",') && notebookStudio.includes('  "audio_transcript",') && notebookStudio.includes('  "caption_track",') && notebookStudio.includes('  "narration_script",') && notebookStudio.includes("Audio overview is active") && notebookStudio.includes("Sesli anlatım hazır"));
addCheck("Notebook Studio media copy avoids fake export claims", notebookStudio.includes("Deck export manifest") && !notebookStudio.includes("PPTX indir") && !notebookStudio.includes("Video hazir"));
addCheck("Notebook Studio slide export contract is wired", api.includes("getExportPreview") && api.includes("/export/preview") && api.includes("exportPack") && api.includes("/export") && types.includes("NotebookExportResultDto") && types.includes("NotebookSlideExportPreviewDto") && types.includes("exportScope") && types.includes("internalConnectionKeys"));
addCheck("Notebook Studio phase scope is typed", types.includes("phaseScope: string[]") && types.includes("ChatResponseMetadata") && types.includes("surface?: string | null") && types.includes("contextType?: string | null"));
addCheck("Notebook Studio export UX is honest", notebookStudio.includes("Export package") && notebookStudio.includes("pptx_not_enabled") && notebookStudio.includes("Safe HTML") && !/fake video|fake pptx|pptx indir/i.test(notebookStudio));
addCheck("Notebook Studio export preview is useful", notebookStudio.includes("Deck outline") && notebookStudio.includes("Kaynak temeli") && notebookStudio.includes("Erisilebilirlik") && notebookStudio.includes("Export scope") && notebookStudio.includes("Checkpoint ready") && notebookStudio.includes("Speaker notes ready"));
...
addCheck("Notebook Studio UI copy is readable", !notebookMojibake.test(notebookStudio) && notebookStudio.includes("Study guide") && notebookStudio.includes("Deck preview") && notebookStudio.includes("UML / Mermaid"));

const sidebar = read("src/components/LeftSidebar.tsx");
const navigation = read("src/lib/appNavigation.ts");
addCheck("Sidebar exposes canonical Learning OS IA", ["Ana Kokpit", "Tutor", "Study Room", "Review / Quiz", "Exam War Room", "Sources / Wiki", "Notebook Studio", "Code IDE", "Progress", "Settings / Safety"].every((label) => navigation.includes(label)) && sidebar.includes("APP_NAV_ITEMS") && !sidebar.includes("Sohbet") && !sidebar.includes("OrkaWiki") && !sidebar.includes("OrkaLM") && !sidebar.includes("Pratik / Tekrar") && !sidebar.includes("TopicReadinessBadge") && !sidebar.includes("labelKey"));
addCheck("Sidebar keeps legacy view compatibility without legacy nav items", navigation.includes('chat: "tutor"') && navigation.includes('wiki: "sources-wiki"') && navigation.includes('orkalm: "notebook"') && navigation.includes('practice: "review"') && navigation.includes('"central-exams": "exams"'));
addCheck("App sidebar exposes logout action", sidebar.includes("Log out") && sidebar.includes("onLogout") && sidebar.includes("logoutLoading") && sidebar.includes("LogOut"));

const centralExams = read("src/components/CentralExamsPanel.tsx");
const centralHome = read("src/pages/Home.tsx");
addCheck("Central exams product shell is visible", navigation.includes("Exam War Room") && navigation.includes('view: "exams"') && (centralHome.includes("central-exams") || centralHome.includes("exams")) && centralExams.includes("Merkezi Sinavlar") && centralExams.includes("KPSS hazirlik iskeleti"));
...
addCheck("Central exams multi-exam shell is visible", centralExams.includes("YKS") && centralExams.includes("LGS") && centralExams.includes("YDS") && centralExams.includes("Hazirlik iskeleti") && centralExams.includes("Question flow") && centralExams.includes("Mini deneme"));
...
addCheck("KPSS pilot exam drill flow is visible", centralExams.includes("KPSS Turkce Paragraf drill") && centralExams.includes("Start exam drill") && centralExams.includes("Cevaplari gonder") && centralExams.includes("Cevap anahtari ve aciklama sadece gonderimden sonra gosterilir"));
...
addCheck("Source and Notebook canonical modes keep Pro contract as entry surface", home.includes("<SourceWikiProPanel") && home.includes("<NotebookStudioProPanel") && home.includes("workspace={") && !home.includes('case "sources-wiki":\n        return (wikiTopicId') && !home.includes('case "notebook":\n        return (wikiTopicId'));
...
addCheck("Chat null-topic first message is protected from topic-load races", home.includes("sessionRequestRef") && home.includes("Do not auto-select the first topic") && !home.includes("setActiveTopic(loaded[0])") && home.includes('m.messageType === "general" ? "text"'));
addCheck("Chat stream fallback avoids duplicate POST after stream starts", chatPanel.includes("let streamStarted = false") && chatPanel.includes("streamStarted = true") && chatPanel.includes("if (streamStarted)") && chatPanel.includes("streamProducedContent") && chatPanel.includes("Bağlantı açıldı ama bu turda görünür yanıt üretilmedi"));
...
addCheck("Chat classroom audio carries surface context", chatMessage.includes("classroomSurface") && chatMessage.includes("classroomSourceId") && chatMessage.includes("wikiPageId={classroomWikiPageId}") && chatMessage.includes("sourceId={classroomSurface === \"orkalm\" ? classroomSourceId : undefined}"));
...
addCheck("Live trace stays behind safe student summary", chatMessage.includes("LiveTutorTrace") && chatMessage.includes("TutorAPI.getSessionTimeline") && chatMessage.includes("Canlı Orka izi") && !chatMessage.includes("Canlı Tutor izi"));
...
addCheck("Tutor chat carries projection rail without heavy workspace surfaces", !chatPanel.includes("ToolCapabilityStrip") && chatPanel.includes("AgentStatusRail") && !chatPanel.includes("ArtifactCanvas") && chatPanel.includes("composer-action-menu-trigger") && agenticWorkspace.includes("workspaceState?.currentPlanStep") && agenticWorkspace.includes("workspaceState?.sourceReadiness"));
...
addCheck("Landing keeps language selector while app shell keeps clean IA", landing.includes("setLanguage") && landing.includes("languages.map") && navigation.includes("Ana Kokpit") && navigation.includes("Notebook Studio") && !sidebar.includes("interface_language"));
addCheck("Sources and Notebook surfaces are wired into app shell", (home.includes('"sources"') || home.includes('"sources-wiki"') || home.includes('"orkalm"')) && home.includes('mode="orkalm"') && navigation.includes("Sources / Wiki") && navigation.includes("Notebook Studio") && messages.includes("Notebook Studio"));
