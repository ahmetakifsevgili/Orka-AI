...
public interface IAgentOrchestrator
{
    ...
    Task<ChatMessageResponse> ProcessMessageAsync(Guid userId, string content, Guid? topicId, Guid? sessionId, bool isPlanMode = false, Guid? focusTopicId = null, string? focusTopicPath = null, string? focusSourceRef = null, CancellationToken ct = default);
    IAsyncEnumerable<string> ProcessMessageStreamAsync(Guid userId, string content, Guid? topicId, Guid? sessionId, bool isPlanMode = false, Guid? focusTopicId = null, string? focusTopicPath = null, string? focusSourceRef = null, CancellationToken ct = default);
    Task EndSessionAsync(Guid sessionId, Guid userId);
}

public interface ITutorAgent
{
    Task<string> GetResponseAsync(Guid userId, string content, Session session, bool isQuizPending, CancellationToken ct = default);
    ...
    /// </summary>
    Task<string> GetDeepPlanWelcomeAsync(Guid userId, string content, Session session, IReadOnlyList<string> planTitles, CancellationToken ct = default);
    Task<string> GetOptionsWelcomeAsync(Guid userId, string content, Session session, CancellationToken ct = default);
    ...
    /// </summary>
    Task<string> GetFirstLessonAsync(string parentTopicTitle, string lessonTitle, IReadOnlyList<string>? curriculumTitles = null, CancellationToken ct = default);
    ...
    /// </summary>
    Task<string> GenerateTopicQuizAsync(string topicTitle, string? researchContext = null, CancellationToken ct = default);

    /// <summary>
    /// Öğrencinin cevabını değerlendirir.
    /// content içinde '[PLAYWRIGHT_PASS_QUIZ]' varsa AI çağrısı yapılmadan DOĞRU döner.
    /// </summary>
    Task<bool> EvaluateQuizAnswerAsync(string question, string answer, CancellationToken ct = default);
}
