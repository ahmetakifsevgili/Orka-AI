...
public sealed class ToolCapabilityService : IToolCapabilityService
{
    ...
    public IReadOnlyList<ToolCapabilityDto> GetCapabilities(bool includeInternal = false)
    {
        var tools = new List<ToolCapabilityDto>
        {
            ...
            PublicProvider("knowledge_entity", "Knowledge entity evidence", "teaching_evidence", "Low", "Wikipedia/Wikidata public APIs.",
                "Stable concept/person/place/event grounding for examples and definitions.", "concept/query", "evidence cards"),
            PublicProvider("geo_context", "Geography evidence", "teaching_evidence", "Low", "Open-Meteo geocoding + OpenTopodata public fallback.",
                "Coordinates, latitude band, elevation and place-based reasoning for lessons.", "place/query", "map context card"),
            PublicProvider("socioeconomic_context", "Socioeconomic evidence", "teaching_evidence", "Medium", "REST Countries + World Bank public APIs.",
                "Population, area, density and public country indicators for real-world tables.", "country/query", "real-world data card"),
            PublicProvider("science_context", "Science evidence", "teaching_evidence", "Medium", "PubChem, USGS, NASA and GBIF public APIs.",
                "Scientific facts, live earthquake examples, astronomy images and species data.", "science concept/query", "science evidence card"),
            PublicProvider("research_context", "Research evidence", "teaching_evidence", "Medium", "OpenAlex, arXiv and OpenLibrary public APIs.",
                "Academic and reading context; not a consensus claim by itself.", "topic/query", "research reading card"),
            PublicProvider("forum_signal", "Forum misconception signal", "teaching_evidence", "Medium", "Stack Exchange + HN Algolia public APIs.",
                "Forum data is used only as a common-confusion signal, never as factual authority.", "topic/error/query", "forum pattern card"),
            ...
            PublicProvider("weather", "Geography context", "external_info", "Medium", "Open-Meteo public fallback; OpenWeatherMap key optional.",
                "Place-based geography context uses coordinates, latitude band, climate cue, and map reasoning. Live weather is only a supporting signal.", "location/coordinates", "geography context"),
