...
public sealed class ToolCapabilityService : IToolCapabilityService
{
    ...
    public IReadOnlyList<ToolCapabilityDto> GetCapabilities(bool includeInternal = false)
    {
        var tools = new List<ToolCapabilityDto>
        {
            ...
            AdminDev("ide_execution", "IDE / code execution tool", "code_execution", "High", "Tools:IdeExecution:Enabled",
                "High-risk Tutor tool. Public code execution remains available only through sandboxed API; SK auto-tool is disabled/admin-dev gated.", "code/language/stdin", "bounded stdout/stderr"),
